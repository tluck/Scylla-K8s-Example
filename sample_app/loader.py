...
if __name__ == "__main__":
    # cluster = Cluster(contact_points=hosts, auth_provider=PlainTextAuthProvider(username=username, password=password))
    try:
        if hosts == ['127.0.0.1']:
            cluster = Cluster(hosts, auth_provider=PlainTextAuthProvider(username=username, password=password))
        else:
