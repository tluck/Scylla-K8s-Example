...
scylla_client_ip="$( kubectl -n ${scyllaNamespace} get service/${clusterName}-client -o='jsonpath={.spec.clusterIP}' )"
external_client_ip=$(kubectl -n ${scyllaNamespace} get svc external-client -o json | jq -r .status.loadBalancer.ingress[0].ip)
...
[connection]
# hostname = 127.0.0.1
# hostname = ${scylla_client_ip}
hostname = ${external_client_ip}
port = 9142
ssl = true
